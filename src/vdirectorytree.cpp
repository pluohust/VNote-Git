...
void VDirectoryTree::contextMenuRequested(QPoint pos)
{
    ...
    if (!item) {
        ...
        QAction *newTodayNoteAct = new QAction(VIconUtils::menuIcon(":/resources/icons/create_note_tb.svg"),
                                          tr("新建日记"),
                                          &menu);
        connect(newTodayNoteAct, &QAction::triggered,
                this, []() {
                    g_mainWin->getFileList()->newDiaryNote();
                });

        menu.addAction(newTodayNoteAct);
