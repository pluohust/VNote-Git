...
int cred_acquire_cb(git_cred** cred, const char* url, const char* username_from_url, unsigned int allow_types, void *payload) {
    std::string username = g_config->getGitName().toStdString();
    std::string password = g_config->getPassword().toStdString();
    git_cred_userpass_plaintext_new(cred, username.c_str(), password.c_str());
    return 0; 
}

int VMainWindow::gitAddAllCommit() {
    errorGit = false;
    git_libgit2_init();
    ...
    std::string gitNoteNoteHome = g_config->getVnoteNotebookFolderPath().toStdString();
    std::string userName = g_config->getGitName().toStdString();
    std::string email = g_config->getGitEmail().toStdString();
    error = git_repository_open(&rep, gitNoteNoteHome.c_str());
    if (error < 0) {
        const git_error *e = giterr_last();
        QString errorInfo = "Error: ";
        errorInfo += QString::number(error) + "/" + QString::number(e->klass) + ":" + e->message;
        errorGit = true;
        errorMessage = errorInfo + " add all and commit";
        goto SHUTDOWN_ADD;
    }
    ...
    git_signature_now(&me, userName.c_str(), email.c_str());
    error = git_commit_create(&new_commit, rep, "HEAD", me, me, "UTF-8", "note", new_tree, parent_count, parents);
...
int VMainWindow::gitPull() {
    ...
    if (git_index_has_conflicts(index)) {
        ...
        git_signature_now(&me, userName.c_str(), email.c_str());
        error = git_commit_create_v(&commit_id, rep, git_reference_name(local_master), me, me, "UTF-8", "note", new_tree, 2, our_commit, their_commit);
...
void VMainWindow::myGitErrorShow() {
    if (errorGit) {
        QMessageBox::information(this, "Git error", errorMessage, QMessageBox::Ok);
    }
}

void VMainWindow::myUpdateGit() {
    if (updateStatus) return;
    else updateStatus = true;
    gitMovie->start();
    std::thread t([this](){
        if (gitPull() < 0 || gitAddAllCommit() < 0 || gitPush() < 0) {
            updateStatus = false;
        }
        gitMovie->stop();
        updateStatus = false;
...
void VMainWindow::whenSaveFile() {
    m_editArea->saveFile();
    if (git_time.elapsed() > 15 * 60 * 1000) {
        git_time.restart();
        emit gitUpdate->trigger();
    }
}
      ...
      m_syncNoteListToCurrentTab(true)
{
    ...
    gitMovie = new QMovie(":/resources/icons/update.gif", QByteArray(), this);
    connect(gitMovie, &QMovie::frameChanged, [this](){
        if (gitMovie && gitUpdate) {
            gitUpdate->setIcon(QIcon(gitMovie->currentPixmap()));
        }
    });
    connect(this, SIGNAL(myGitErrorSignal()), this, SLOT(myGitErrorShow()));
    git_time.start();
}
...
QToolBar *VMainWindow::initNoteToolBar(QSize p_iconSize)
{
    ...
    gitUpdate = new QAction(VIconUtils::toolButtonIcon(":/resources/icons/update.gif"),
                                             tr("Git Update"),
                                             this);
    connect(gitUpdate, &QAction::triggered, this, &VMainWindow::myUpdateGit);
    ...
    m_noteToolBar->addAction(gitUpdate);
